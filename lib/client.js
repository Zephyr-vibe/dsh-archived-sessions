	...
	factory: (require) => {
		...
		function ArchivedSessionsSection({ useSessions, useWorkspaces, refresh, t }) {
			...
			const groups = (0, react.useMemo)(() => {
				...
				const childrenOf = new Map();
				for (const row of allRows) {
					if (!row.subagent || row.parentId === void 0 || !allRows.some((r) => r.id === row.parentId)) continue;
					const list = childrenOf.get(row.parentId) ?? [];
					list.push(row);
					childrenOf.set(row.parentId, list);
				}
				...
				const lineageOf = (id) => {
					...
					if (kids !== void 0) for (const kid of kids) ids.push(...lineageOf(kid.id));
					return ids;
				};
				...
				const byWorkspace = workspaceItems.map((ws) => {
					const tops = (ws.sessionIds ?? []).map((id) => allRows.find((row) => row.id === id)).filter((row) => row !== void 0);
				...
				/** 未分组 = 顶层会话（含其子代理），且不归属任何工作区；已归档的由上层过滤。 */
				const ungrouped = allRows.filter((row) => !row.subagent && !accounted.has(row.id));
				// 未分组同样置顶当前会话
				ungrouped.sort((a, b) => {
					if (a.current !== b.current) return a.current ? -1 : 1;
					return (b.updatedAt ?? 0) - (a.updatedAt ?? 0);
				...
				return result;
			}, [tab, viewMode, workspaceItems, allRows, expandedParents, t]);
