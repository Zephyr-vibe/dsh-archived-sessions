window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const pcss = {
			...
			"headerDelete": "aRchv_headerDelete"
		};
		//#endregion
		//#region locales
		const zh = {
		 ...
		 * 默认都不勾选——只删所选会话本身。 */
		function DeleteSessionAction({ sessionId, t, useSessions, onDelete }) {
			...
			const sessionCwd = typeof byId[sessionId]?.cwd === "string" ? byId[sessionId].cwd : void 0;
			...
			const openConfirm = async () => {
				...
				try {
					...
					for (const id of [sessionId, ...kidsSet]) {
						try {
							const d = await api("details", { sessionId: id });
							if (rootCwd === null && typeof d?.cwd === "string") rootCwd = d.cwd;
							for (const f of d?.files ?? []) files.add(f.path);
						} catch {
							// 单个会话详情失败不影响其他
						}
			...
			const confirmDelete = async () => {
				...
				setError(null);
				try {
					await onDelete(sessionId, { subagentIds: [...subagentIds], filePaths: [...filePaths] });
					setOpen(false);
				} catch (reason) {
					setError(reason instanceof Error ? reason.message : String(reason));
				} finally {
								...
								(0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										(0, react_jsx_runtime.jsxs)("label", {
											style: { display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--dsw-alias-label-secondary)", cursor: "pointer" },
											children: [
												(0, react_jsx_runtime.jsx)("input", { type: "checkbox", checked: subagentIds.size > 0, disabled: deleting, onChange: (e) => setSubagentIds(e.target.checked ? new Set(allSubagentIds) : new Set()) }),
												t("deleteCascade"),
												(0, react_jsx_runtime.jsx)("button", {
													type: "button",
													"aria-label": t("deleteDetail"),
										...
										(0, react_jsx_runtime.jsxs)("label", {
											style: { display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--dsw-alias-label-secondary)", cursor: "pointer" },
											children: [
												(0, react_jsx_runtime.jsx)("input", { type: "checkbox", checked: filePaths.size > 0, disabled: deleting, onChange: (e) => setFilePaths(e.target.checked ? new Set(allFiles) : new Set()) }),
												t("deleteFiles"),
												(0, react_jsx_runtime.jsx)("button", {
													type: "button",
													"aria-label": t("deleteDetail"),
