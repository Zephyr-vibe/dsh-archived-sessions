 ...
 * semantics via detach, `sessionPersistence.locate` for the artifact path) and
 * degrades gracefully on a stock Harness. Agent teardown (AgentHandle.dispose)
 * is a creator-held capability unavailable from this root context, so deletion
 * flushes buffered writes and removes the persisted artifact — same behavior
 * as the official delete handler when dispose is unreachable.
 */
...
async function buildDetails(ctx, sessionId) {
	...
	let sizeBytes = null;
	// 审查 M1 修复: 已装 jsonl 后端（0.1.0-rc.6）没有 artifactInfo；改用官方
	// locate() 定位日志文件后 stat 其字节数（"占用空间"= 会话记录文件大小）。
	// 保留 artifactInfo 分支以便未来后端提供该原语时自动升级。
	...
	} else if (persistence !== void 0 && typeof persistence.locate === "function" && meta !== void 0) {
		try {
...
async function deleteSessionSingle(ctx, sessionId) {
	...
	// 审查 M2 修复: 已装 jsonl 后端没有按字符串入参的 remove（coordinator 的
	// remove(entry) 是私有成员、签名不同），因此删除统一走 locate+rm 主路径：
	// locate() 返回每会话各自目录下的日志文件，dirname 即该会话专属目录，
	// 递归删除恰好只移除本会话的记录——非级联语义在此布局下天然成立。
	// 围栏校验：目录必须严格位于会话根目录内，防第三方/损坏后端把递归 rm
	// 指向整个会话库（或更上层）。
	{
		const location = persistence?.locate?.(meta);
		if (location !== void 0 && typeof location.path === "string") {
...
async function deleteSession(ctx, sessionId) {
	...
	if (agent !== void 0) {
		// Best-effort flush of buffered writes, matching the official
		// deleteSession handler's durability step.
		// 审查 M10 修复: 官方 teardown 只有创建者持有的 AgentHandle.dispose()
		// （dsh-agent types: dispose 是 CAPABILITY，ctx.agents.get(id) 返回裸
		// Agent 无 dispose；agentLoop 服务也无 disposeAgent 方法），根 context
		// 拿不到该能力，故不存在可用的 dispose 调用——删除原 disposeAgent 死
		// 分支。运行中（running）会话已在上方 409 拒绝；空闲（idle）会话
		// 直接移除持久化记录即可，与官方"无法 dispose 时跳过并删除"一致。
		try {
			const sessions = ctx.get("sessions");
...
async function deleteFile(ctx, path, sessionId) {
	...
	// M6: 归属校验——path 必须是该会话产出文件列表之一。sessionId 必填
	// （审查 S1 加固：即使绕过路由层校验，缺失时也直接拒绝，不允许退化
	// 到仅工作区围栏——围栏无法区分"该会话产出"与"工作区内任意文件"）。
	if (typeof sessionId !== "string" || sessionId === "") {
		const error = new Error("delete-file 需要 sessionId 做产出文件归属校验");
		error.status = 400;
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (method === "delete-file") {
					...
					// 安全修复（审查 S1）: sessionId 必填——缺失时无法做产出文件归属
					// 校验，仅靠工作区围栏无法阻止删除工作区内任意文件（围栏只要求
					// 目标位于任一注册工作区根内，不区分是否该会话产出）。
					const ownerSessionId = typeof payload.sessionId === "string" ? payload.sessionId : "";
					if (ownerSessionId === "" || ownerSessionId.length > 200) {
						writeJson(res, 400, { ok: false, error: { code: "bad-request", message: ownerSessionId === "" ? "sessionId is required" : "sessionId is too long" } });
						return;
					}
